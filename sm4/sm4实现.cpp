#include<iostream>
#define u8 unsigned char
#define u32 unsigned long

const u8 Sbox[256] = {
	
};

const u32 FK[4] = {
	0xa3b1bac6, 0x56aa3350, 0x677d9197, 0xb27022dc
};


const u32 CK[32] = {
	0x00070e15, 0x1c232a31, 0x383f464d, 0x545b6269,
	0x70777e85, 0x8c939aa1, 0xa8afb6bd, 0xc4cbd2d9,
	0xe0e7eef5, 0xfc030a11, 0x181f262d, 0x343b4249,
	0x50575e65, 0x6c737a81, 0x888f969d, 0xa4abb2b9,
	0xc0c7ced5, 0xdce3eaf1, 0xf8ff060d, 0x141b2229,
	0x30373e45, 0x4c535a61, 0x686f767d, 0x848b9299,
	0xa0a7aeb5, 0xbcc3cad1, 0xd8dfe6ed, 0xf4fb0209,
	0x10171e25, 0x2c333a41, 0x484f565d, 0x646b7279
};



u32 functionB(u32 b) {
	u8 a[4];
	a[0] = b / 0x1000000;
	a[1] = b / 0x10000;
	a[2] = b / 0x100;
	a[3] = b;
	b = Sbox[a[0]] * 0x1000000 + Sbox[a[1]] * 0x10000 + Sbox[a[2]] * 0x100 + Sbox[a[3]];
	return b;
}


u32 loopLeft(u32 a, short length) {
	u32 temp1 = a << length;
	u32 temp2 = a >> (32 - length);
	a = temp1 + temp2;
	return a;
}


u32 functionL1(u32 a) {
	return a ^ loopLeft(a, 2) ^ loopLeft(a, 10) ^ loopLeft(a, 18) ^ loopLeft(a, 24);
}


u32 functionL2(u32 a) {
	return a ^ loopLeft(a, 13) ^ loopLeft(a, 23);
}


u32 functionT(u32 a, short mode) {
	u32 temp = functionB(a);
	return mode == 1 ? functionL1(temp) : functionL2(temp);
}


void extendFirst(u32 MK[], u32 K[]) {
extendFirst_label1:for (int i = 0; i < 4; i++) {
	K[i] = MK[i] ^ FK[i];
}
}


void extendSecond(u32 RK[], u32 K[]) {
	short i;
extendSecond_label2:for (i = 0; i < 32; i++) {
	K[i % 4] = K[i % 4] ^ functionT(K[(i + 1) % 4] ^ K[(i + 2) % 4] ^ K[(i + 3) % 4] ^ CK[i], 2);
	RK[i] = K[i % 4];
}
}


void getRK(u32 MK[], u32 K[], u32 RK[]) {
	extendFirst(MK, K);
	extendSecond(RK, K);
}


void iterate32(u32 X[4], u32 RK[]) {
	short i;
iterate32_label3:for (i = 0; i < 32; i++) {
	X[i % 4] = X[i % 4] ^ functionT(X[(i + 1) % 4] ^ X[(i + 2) % 4] ^ X[(i + 3) % 4] ^ RK[i], 1);
}
}


void reverse(u32 X[], u32 Y[]) {
	short i;
reverse_label4:for (i = 0; i < 4; i++) {
	Y[i] = X[3 - i];
}
}


void encryptSM4(u32 X[], u32 RK[], u32 Y[]) {

	iterate32(X, RK);
	reverse(X, Y);
}
void runtest(u32 X[4], u32 MK[4])
{
	u32 RK[32]; // 轮密钥
	u32 K[4]; // 中间数据
	u32 Y[4]; // 密文
	printf("明文：");
	printf("%8x %8x %8x %8x\n", X[0], X[1], X[2], X[3]);
	printf("密钥：");
	printf("%8x %8x %8x %8x\n", MK[0], MK[1], MK[2], MK[3]);
	getRK(MK, K, RK);
	printf("密文:\n");
	encryptSM4(X, RK, Y);
	printf("%08x %08x %08x %08x\n", Y[0], Y[1], Y[2], Y[3]);
}
int main(void) {
	u32 X[4] = { 0x01234567,0x89abcdef,0xfedcba98,0x76543210 }; // 明文
	u32 MK[4] = { 0x01234567,0x89abcdef,0xfedcba98,0x76543210 }; // 密钥
	runtest(X, MK);

}
